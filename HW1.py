...
def print_rangoli(size):
    ...
    matrice = []
    for i in range(size):
        #parte sinistra : parti dalla lettere size fino ad a 
        sin = lettere[size-1:size-i-1:-1]
        #riga centrale  
        cen = lettere[size-i-1]  
        des = lettere[size-i:size] 
        #unisci le tre componenti
        riga = "-".join(sin + cen + des)
        #mettili nella matrice
        matrice.append(riga.center(4*size - 3, "-"))
    #stampala dritta
    for _ in matrice:
        print(_)
